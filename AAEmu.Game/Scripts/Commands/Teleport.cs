                    ...
                    else
                    {
                        ...
                        if (height == 0f)
                        {
                            character.SendMessage("|cFFFF0000[Teleport] Target height was |cFFFFFFFFzero|cFFFF0000. " +
                                ...
                                "X:" + character.LocalPingPosition.Position1[1].X.ToString("0.0") + " Y:" + character.LocalPingPosition.Position1[1].Y.ToString("0.0") + "|r");
                        }
                        else
                        {
                            height += 2.5f; // compensate a bit for terrain irregularities
                            character.SendMessage("Teleporting to |cFFFFFFFFX:" + character.LocalPingPosition.Position1[1].X + " Y:" + character.LocalPingPosition.Position1[1].Y + " Z:" + height + "|r");
                            character.DisabledSetPosition = true;
                            character.SendPacket(new SCUnitTeleportPacket(0, 0, character.LocalPingPosition.Position1[1].X, character.LocalPingPosition.Position1[1].Y, height, 0));
                        }
                    }
                }
                else
