                ...
                using (var command = connection.CreateCommand())
                {
                    ...
                    using (var reader = new SQLiteWrapperReader(command.ExecuteReader()))
                    {
                        while (reader.Read())
                        {
                            ...
                            var tblColumnName = reader.GetString("tbl_column_name");
                            var idx = reader.GetInt64("idx");
                            var lngStr = reader.IsDBNull(DefaultLanguage) ? "NotLocalized" : reader.GetString(DefaultLanguage);
                            AddTranslation(tblName, tblColumnName, idx, lngStr);
                        }
                    }
                }
            }
